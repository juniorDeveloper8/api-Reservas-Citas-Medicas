...
namespace Integrador.Logica.DoctorService
{
    public class InsertDoctorService
    {
        ...
        public InsertDoctorService(DbIntegradorContext context)
        {
            _context = context;
        }

        public async Task<bool> InsertarDoctor(DoctorDTO doctorDTO)
        {
            try
            {
                ...
                await _context.Database.ExecuteSqlRawAsync("EXEC InsertarDoctor @nombre, @apellido, @tipoDocumento, @dni, @correo, @username, @psw, @celular",
                    nombreParam, apellidoParam, tipoDocumentoParam, dniParam, correoParam, usernameParam, pswParam, celularParam);
